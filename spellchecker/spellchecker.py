...
class SpellChecker(object):
    ...
    def __init__(self, language="en", local_dictionary=None, distance=2):
        ...
        if local_dictionary:
            ...
            filename = "{}.dict".format(language.lower())
            here = os.path.dirname(__file__)
            full_filename = "./resources/"+filename  #os.path.join(here, "resources", filename)
...
#            with open(language+'.dict','wb') as f :
#              fpk = pk.Pickler(f)
#              fpk.dump(self._word_frequency._dictionary)



#            with open(language+'.txt','w') as f :

#              for k in self._word_frequency._dictionary.keys() :
#                f.write(k+"\n")
    ...
    def correction(self, word):
        ...
        select=max(self.candidates(word), key=self.word_probability)
        if 'ç' in select :
          print("ç exist")
        return select
...
class WordFrequency(object):
    ...
    def reload_dictionary(self, full_filename) :

      dico={}
      with open(full_filename,"rb") as f :
        fupk=pk.Unpickler(f)
        dico=fupk.load()
#      print('nb items : ',len(dico))
      print('nb dico items : ',len(dico))
      return dico
...
#        self._dictionary.update(json.loads(data.lower(), encoding=encoding))
        self._dictionary.update(self.reload_dictionary(filename))
